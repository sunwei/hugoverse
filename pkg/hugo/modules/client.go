...
func (c *Client) Vendor() error {
	...
	for _, t := range tc.AllModules {
		...
		if !t.IsGoMod() && !t.Vendor() {
			...
			// theme directory, see https://github.com/sunwei/hugoverse/pkg/hugo/issues/5993
			continue
		}

		// See https://github.com/sunwei/hugoverse/pkg/hugo/issues/8239
...
func (c *Client) runGo(
	...
	if err := cmd.Run(); err != nil {
		...
		if strings.Contains(stderr.String(), "invalid version: unknown revision") {
			// See https://github.com/sunwei/hugoverse/pkg/hugo/issues/6825
...
require github.com/sunwei/hugoverse/pkg/hugo-mod-jslibs/instantpage v0.5.1
